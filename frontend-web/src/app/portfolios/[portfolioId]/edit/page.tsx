...
export default function EditPortfolioPage() {
  ...
  React.useEffect(() => {
    ...
    async function fetchPortfolioData() {
      ...
      try {
        ...
        // The backend might return JSON strings for object fields. We need to parse them.
        // Or, it might return actual objects if Spring's Jackson is configured well.
        // For safety, let's try to parse if they are strings.
        ...
        form.reset({ // Populate the form with fetched data
        ...
        if (err.message === "Portfolio not found.") {
            // Optionally redirect or show a specific "not found" UI
  ...
  async function onSubmit(data: EditPortfolioFormValues) {
    ...
    try {
      ...
      // The backend expects JSON strings for these fields if they are 'Object' type
      // Ensure we stringify them before sending
      const payload = {
        ...data,
      ...
      <div className="flex min-h-screen flex-col bg-gray-50 dark:bg-gray-900">
        ...
        <main className="flex-1 container max-w-4xl mx-auto py-8 md:py-12">
          <div className="flex flex-col space-y-2 mb-8">
            <Skeleton className="h-6 w-24 mb-2" /> {/* Back button */}
  ...
  // Error State UI (if initial fetch failed critically)
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="flex flex-col space-y-2 mb-8">
              <h1 className={`text-3xl font-bold tracking-tight ${primaryTextColor}`}>
                My Portfolio: <span className="text-[#c89b3c]">{initialData?.title || "Loading..."}</span>
              </h1>
            ...
            {/* Display any top-level submission error */}
            {error && (
                            ...
                            <div key={categoryItem.id} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg space-y-4 bg-slate-50 dark:bg-slate-800/30">
                                <div className="flex justify-between items-center">
                                    ...
                                    <Button 
                                        ...
                                        variant="ghost" 
                                        size="sm"
                                        className="ml-4 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-100 dark:hover:bg-red-700/50 p-1 h-auto self-end mb-1" // Adjusted alignment
            ...
            <div className="flex justify-end mt-8">
                ...
                >
                  ...
                  {isSubmittingForm ? "Saving Portfolio..." : "Save Portfolio"}
                </Button>
            </div>
          </form>
        </Form>
...
// Helper component for nested Skill Items array
      ...
      {fields.map((item, skillIndex) => (
        <div key={item.id} className="flex items-end gap-3 relative pt-2">
          ...
          <Button 
            ...
            disabled={isSubmittingForm || fields.length <=1} // Prevent removing the last item if you want at least one skill per category
          >
            <Trash2 className="h-4 w-4" />
